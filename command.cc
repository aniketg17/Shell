...
void Command::execute() {
  ...
  if ( !strcmp(cmd->c_str(),"exit") || !strcmp(cmd->c_str(), "q") || !strcmp(cmd->c_str(), "leave") || !strcmp(cmd->c_str(), "return")) {
    printf( "Good bye!!\n");
    exit(1);
  }
  ...
  for (int i=0; i<numsimplecommands; i++) {
    ...
    /*if(!strcmp(_simpleCommands[i]->_arguments[0]->c_str(), "source")) {
      ...
      std::string line;
      std::ifstream fd; //input file stream
      fd.open(_simpleCommands[i]->_arguments[1]->c_str());
      std::getline(fd, line); //reads line from stream
      fd.close();
      ...
      int fdpipein[2];
      pipe(fdpipein);
      write(fdpipein[1], line.c_str(), strlen(line.c_str()));
      write(fdpipein[1], "\n", 1);
      close(fdpipein[1]);
      ...
      if (ret2 == 0) {
        ...
        argv[0] = (char*) "/proc/self/exe"; //link to currently running process
        argv[1] = NULL;
        execvp(argv[0], argv);
        exit(1);
      } else if(ret2 < 0) {
